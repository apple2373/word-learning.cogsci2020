...
def check_gitstatus():
    ...
    if sha is None:
        sha = subprocess.run(["git","rev-parse","HEAD"], stdout=PIPE, stderr=subprocess.PIPE).stdout.decode("utf-8").strip() 
    print("git hash",sha)    
    
    if status is None:
        status = subprocess.run(["git","status"], stdout=PIPE, stderr=subprocess.PIPE).stdout.decode("utf-8").strip()
